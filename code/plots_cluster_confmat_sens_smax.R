...
res1<-readRDS(file = "outs/simest/Smax_sensitivity/res_smax1.rds")
res2<-readRDS(file = "outs/simest/Smax_sensitivity/res_smax2.rds")
res240<-readRDS(file = "outs/simest/Smax_sensitivity/res_smax240.rds")
...
aic=subset(restmb, parameter=='AIC'&method=='MLE')
bic=subset(restmb, parameter=='BIC'&method=='MLE')
lfo=subset(restmb, parameter=='LFO'&method=='MLE')
...
pa_aic
ggsave("../Best-Practices-time-varying-salmon-SR-models/figures/confusion_matrices/sens_smax/AIC_MLE_senssmax.png", plot=pa_aic)
...
pa_bic=ggplot(data =  conf_matrix, mapping = aes(x = OM, y = EM)) +
  ...
  xlab("Simulation Scenario")+ylab("Estimation Model")
pa_bic
ggsave("../Best-Practices-time-varying-salmon-SR-models/figures/confusion_matrices/sens_smax/BIC_MLE_senssmax.png", plot=pa_bic)



p_lfo=ggplot(data =  conf_matrix, mapping = aes(x = OM, y = EM)) +
...
#calculate these 
conf_matrix$diffsmax_ch<- dplyr::case_match(conf_matrix$OM,
...
conf_matrix$diffsmax<- dplyr::case_match(conf_matrix$OM,
...
conf_matrix$type<- dplyr::case_match(conf_matrix$OM,
...
conf_matrix_right<-conf_matrix[conf_matrix$EM=="dynamic.b",]

lineAIC<-ggplot(conf_matrix_right)+
geom_point(aes(x=diffsmax,y=w_AIC,color=type),size=3)+
geom_line(aes(x=diffsmax,y=w_AIC,color=type,linetype=type),size=1.2)+
scale_colour_manual(values = c("#95D840FF","#482677FF", "#95D840FF", "#482677FF"))+
scale_linetype_manual(values = c(1,1,3,3))+
ylab("% of correct model assignment with AIC")+
xlab("magnitude of change: multiplication factor for Smax")+
...
geom_line(aes(x=diffsmax,y=BIC,color=type,linetype=type),size=1.2)+
scale_colour_manual(values = c("#95D840FF","#482677FF", "#95D840FF", "#482677FF"))+
scale_linetype_manual(values = c(1,1,3,3))+
