...
head(res)
ressel<-res[res$scenario%in%c("stationary",
  "decLinearProd",  
  "decLinearCap", 
 "regimeProd",
 "regimeCap")&res$model%in%c("simple","autocorr", "rwa", "rwb", "rwab", "rwa_last3", "rwb_last3", "rwab_last3", 
  "rwa_last5", "rwb_last5", "rwab_last5" ),]

aic=subset(ressel,parameter=='AIC'&method=='MLE')
bic=subset(ressel,parameter=='BIC'&method=='MLE')
lfo=subset(ressel,parameter=='LFO'&method=='MLE')
...
for(a in seq_along(scn)){
  ...
  aic_set[[a]]=tidyr::spread(aica[,-9],key=model,value=mode)
  aic_set[[a]]=aic_set[[a]][c(12,8,9,11,10)] #reorder estimation models

  sc1=apply(aic_set[[a]],1,which.min)
  cn1[[a]]=summary(factor(sc1,levels=seq(1:ncol(aic_set[[a]]))))/1000

  bica=subset(bic,scenario==scn[a])
  bic_set[[a]]=tidyr::spread(bica[,-9],key=model,value=mode)
  bic_set[[a]]=bic_set[[a]][c(12,8,9,11,10)] #reorder estimation models

  sc2=apply(bic_set[[a]],1,which.min)
  cn2[[a]]=summary(factor(sc2,levels=seq(1:ncol(bic_set[[a]]))))/1000

  lfoa=subset(lfo,scenario==scn[a])
  lfo_set[[a]]=tidyr::spread(lfoa[,-9],key=model,value=mode)
  lfo_set[[a]]=lfo_set[[a]][c(12,8,9,11,10)] #reorder estimation models
...
reslfo<-readRDS(file = "outs/simest/resstanloo.rds")
head(reslfo)
...
for(a in seq_along(scn)){
  ...
  cn3[[a]]=summary(factor(sc3,levels=seq(1:ncol(lfomcmc_set[[a]]))))/1000
...
head(res_siglow)
unique(res_siglow$scenario)
unique(res_siglow$model)

ressel_siglow<-res_siglow[res_siglow$scenario%in%c("sigmalow_stationary", 
  "sigmalow_decLinearProd",  
  "sigmalow_decLinearCap", 
 "sigmalow_regimeProd",
 "sigmalow_regimeCap")&res_siglow$model%in%c("simple", "autocorr",
 "rwa", "rwb", "rwab", 
...
aic_siglow=subset(ressel_siglow, parameter=='AIC'&method=='MLE')
bic_siglow=subset(ressel_siglow, parameter=='BIC'&method=='MLE')
lfo_siglow=subset(ressel_siglow, parameter=='LFO'&method=='MLE')
...
conf_matrix$eqem_om <- dplyr::recode(conf_matrix$OM, 
    "sigmalow_stationary"="stationary",
     ...
     "sigmalow_regimeProd"="dynamic.a",            
     "sigmalow_regimeCap"="dynamic.b",             
...
conf_matrix$OM <- dplyr::recode(conf_matrix$OM, 
    "sigmalow_stationary"="stationary",
...
conf_matrix$OM<-factor(conf_matrix$OM, 
    levels=c("stationary", 
        "decLinearProd", 
        "decLinearCap", 
        "regimeProd", 
