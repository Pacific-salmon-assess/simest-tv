...
for(a in seq_along(scn)){
  ...
  #fix NAs for model weights
  aic_set[[a]]=apply(aic_set[[a]],2,tidyr::replace_na,999) #turn all NAs into very high values (improbable)
  #head(aic_set[[a]])
  sc1=apply(aic_set[[a]],1,which.min)
  w1=t(apply(aic_set[[a]],1,samEst::model_weights,form='AIC',type='full'))
  wb1=apply(w1,1,max)
  ...
  #fix NAs for model weights
  bic_set[[a]]=apply(bic_set[[a]],2,tidyr::replace_na,999) #turn all NAs into very high values (improbable)
  
  sc2=apply(bic_set[[a]],1,which.min)
  w2=t(apply(bic_set[[a]],1,samEst::model_weights,form='AIC',type='full'))
  wb2=apply(w2,1,max)
  ...
  #fix NAs for model weights
  lfo_set[[a]]=apply(lfo_set[[a]],2,tidyr::replace_na,-999) #turn all NAs into very low LL values (improbable)
  
  sc3=apply(lfo_set[[a]],1,which.max)
  cn3[[a]]=summary(factor(sc3,levels=seq(1:ncol(lfo_set[[a]]))))/ nlfosim
  w3=t(apply(lfo_set[[a]],1,mw_tmblfo))
  wb3=apply(w3,1,max)
