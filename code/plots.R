...
#todo - summarize pbias for each iteration 
  ...
  #mean by iteration
  dfmpbias<-aggregate(dfpbias$pbias, list(model=dfpbias$model,
    method=dfpbias$method,iteration=dfpbias$iteration,parameter=dfpbias$parameter),
     mean)

  dfmpbias$convergence <- aggregate(dfpbias$convergence, list(model=dfpbias$model,
    method=dfpbias$method,iteration=dfpbias$iteration,parameter=dfpbias$parameter),
     function(x)sum(x,na.rm=T))$x
  
  #Remove hmm average
  dfmpbias <- dfmpbias[!(dfmpbias$model %in% c("hmma_average","hmmb_average",
    "hmmab_average","hmmabhc_average" )),]

  dfmpbias$model <- factor(dfmpbias$model, levels=c("simple","autocorr","rwa",
  "rwb","rwab","hmma_regime","hmmb_regime", "hmmab_regime", "hmmabhc_regime"))
 ...
 head(dfmpbias)
  ...
  dfrmse <- dfrmse[!(dfrmse$model %in% c("hmma_average","hmmb_average",
    "hmmab_average","hmmabhc_average" )),]

  dfrmse$model <- factor(dfrmse$model, levels=c("simple","autocorr","rwa",
  "rwb","rwab","hmma_regime","hmmb_regime", "hmmab_regime", "hmmabhc_regime"
   ))
